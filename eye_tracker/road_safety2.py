...
##              ret,thresh_right = cv2.threshold(gray_right,THRESH_CONST,255,cv2.THRESH_BINARY_INV)
        ...
        if ear < EYE_AR_THRESH:
            ...
            if (timer_started == False):
                ...
                if (timeDiff > last_sec):
                    last_sec = last_sec + 1
                    if (last_sec == 45):
                        callTwilio()
                        # os.system(
                        #     "python C:\\Users\\apoor\\OneDrive\\Documents\\GitHub\\blinkNdo\\road_safety_twilio.py")
                    if (last_sec >= 15):
                        print('CALLING AMBULANCE IN ' +
                              str(45 - last_sec) + 's')
                        playsound('./beep-07.mp3')
                    else:
                        print('EYES CLOSED FOR ' + str(last_sec) + 's')
